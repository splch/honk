...
kernel/vm.go   hypervisor demo: `vm` (M11) + `vm timer` (M12) + `vm paging` (two-stage paging) + `vm dbcn` (read guest memory via SBI DBCN)
...
`vm dbcn`, `reset --confirm`, and `curl http://localhost:8080/`. **Phase D (display + GUI) is
