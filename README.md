...
kernel/vm.go   hypervisor demos: `vm`/`timer` (M11/M12) + `paging` + `dbcn` (read guest mem) + `mmio` (trap-and-emulate) + `irq` (interrupt injection) + `virtio` (split-virtqueue device backend)
...
`vm dbcn`, `vm mmio`, `vm irq`, `vm virtio`, `reset --confirm`, and `curl
